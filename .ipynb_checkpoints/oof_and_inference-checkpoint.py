...
if __name__ == '__main__':
    ...
    model.to(device)
    states = [torch.load(f'{config.MODEL_OUTPUT_PATH}{config.MODEL_LOAD_FOR_INFER}_{config.MODEL_NAME}_fold{fold}_bs{bs}_size{config.IMAGE_SIZE[0]}_mixup{config.MIXUP}_aug{config.AUG}_dt{config.DATETIME}.pth') for fold in range(config.FOLDS)]
    ...
    for fold in range(config.FOLDS):
        ...
        _oof_df.to_csv(f'{config.LOG_DIR}{config.MODEL_LOAD_FOR_INFER}_oof_df_{config.MODEL_NAME}_fold{fold}_bs{bs}_size{config.IMAGE_SIZE[0]}_mixup{config.MIXUP}_aug{config.AUG}_dt{config.DATETIME}.csv', index = False)
        oof_df = pd.concat([oof_df, _oof_df])

    oof_df.to_csv(f'{config.LOG_DIR}{config.MODEL_LOAD_FOR_INFER}_oof_df_{config.MODEL_NAME}_bs{bs}_size{config.IMAGE_SIZE[0]}_mixup{config.MIXUP}_aug{config.AUG}_dt{config.DATETIME}.csv', index = False)
    ...
    if config.INFER == True:
        ...
        inference_df[['id', 'target']].to_csv(f'{config.LOG_DIR}submission_cv{oof_auc}_{config.MODEL_LOAD_FOR_INFER}_{config.MODEL_NAME}_bs{bs}_size{config.IMAGE_SIZE[0]}_mixup{config.MIXUP}_aug{config.AUG}_dt{config.DATETIME}.csv', index=False)
