...
class Net_ArcFace(nn.Module):
    ...
    def forward(self, x):
        ...
        linear_layer_logits = self.linear_layer(backbone_logits)
        batch_norm_logits = self.batch_norm_layer(linear_layer_logits)
        prelu_logits = self.prelu(batch_norm_logits)
        arcface_logits = self.arcface_logits(batch_norm_logits)
